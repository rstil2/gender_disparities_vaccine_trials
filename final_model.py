...
for disease in diseases:
    ...
    # Stack chain and draw dimensions to get all samples
    samples = disease_predictions.values.flatten()
    ci_data[disease] = {
...
for disease in diseases:
    ...
    lower = ci_data[disease]['lower']
    upper = ci_data[disease]['upper']
...
for disease, color in zip(diseases, colors):
    ...
    ax2.plot(predictions.year, median, color=color, zorder=3, label=f'{disease} (Projected)')
